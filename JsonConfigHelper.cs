...
namespace AppGroup
{
    ...
    public class JsonConfigHelper
    {
        ...
        public static string GetDefaultConfigPath(string fileName = "appgroups.json")
        {
            ...
            return Path.Combine(AppPaths.AppDataFolder, fileName);
        }
                            ...
                            foreach (var pathEntry in paths)
                            {
                                ...
                                allTasks.Add(Task.Run(() =>
                                {
                                    try
                                    {
                                        ...
                                        if (launchPath.Equals(@"shell:AppsFolder\Microsoft.Windows.Explorer", StringComparison.OrdinalIgnoreCase))
                                        {
                                            launchPath = "explorer.exe";
                                            if (string.IsNullOrEmpty(launchArgs))
                                                launchArgs = "shell:MyComputerFolder";
                                        }
