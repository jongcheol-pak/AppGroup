...
**수행한 작업 요약**
- 프로젝트 전체 기능에 대한 사용자 도움말 문서 신규 작성

**변경된 파일 목록**:
- `docs/readme/help.md` (신규) - 사용자 도움말 문서
- `README.md` (수정) - 상세 문서 섹션에 도움말 링크 추가
- `notes.md` (수정) - 작업 기록 추가
...
### 부팅 자동실행 시 메인창 표시 버그 수정

**문제**: 앱이 이미 실행 중인 상태에서 부팅 시 MSIX StartupTask로 자동실행되면, 트레이에만 표시되어야 하는데 메인창이 전면으로 표시됨

**원인**: `Program.Main()`의 정적 `HasSilentFlag` 메서드가 `--silent` 명령줄 인수만 확인하고, MSIX `StartupTask` 활성화 종류를 감지하지 못함. `App.xaml.cs`의 인스턴스 `HasSilentFlag`에는 `IsStartedByStartupTask()` 체크가 있었으나, `Program.Main()`에서 이미 기존 윈도우를 찾아 `SetForegroundWindow`+`ShowWindow(SW_RESTORE)`를 호출하고 `return`하므로 `App` 생성자까지 도달하지 않음

**수정**: `Program.cs`의 정적 `HasSilentFlag`에 `AppInstance.GetCurrent().GetActivatedEventArgs().Kind == ExtendedActivationKind.StartupTask` 체크 추가

**변경 파일**:
- `Program.cs` - 정적 `HasSilentFlag`에 StartupTask 감지 로직 추가
...
### MSIX VFS 호환을 위한 Groups 폴더 분리

**작업 일자**: 2026-02-14

**수행한 작업 요약**
- MS Store 배포를 위해 `unvirtualizedResources` 제한 케이퍼빌리티 제거
- Shell 접근 필요 파일(.lnk, .ico)을 비가상화 경로(`%USERPROFILE%\AppGroup\Groups`)로 분리
- 설정/아이콘 캐시 등 앱 전용 파일은 기존 VFS 경로 유지
- 설정 화면에서 Groups 데이터 폴더 변경 기능 추가 (FolderPicker + 가상화 경로 검증)
- 아이콘 파일명 안정화 (타임스탬프 제거, SHChangeNotify로 Shell 캐시 갱신)
- 백업/복원 시 새 GroupsFolder 경로 대응
- 5개 언어(en-US, ko-KR, ja-JP, zh-CN, zh-TW) 리소스 문자열 추가

**변경된 파일 목록**
- `AppPaths.cs` - GroupsFolder 분리, 마이그레이션 로직, UpdateGroupIconPaths 유틸
- `SettingsHelper.cs` - GroupsDataPath 속성, LoadGroupsDataPathSync() 동기 메서드
- `Program.cs` - Groups 경로 초기화 순서 변경
- `Package.appxmanifest` - unvirtualizedResources, desktop6 네임스페이스 제거
- `View/EditGroupWindow.xaml.cs` - 아이콘 파일명 타임스탬프 제거, Shell 알림 추가
- `View/PopupWindow.xaml.cs` - Shell 아이콘 캐시 알림 추가
- `TaskbarManager.cs` - NotifyShellIconChange 공개 메서드 추가
- `View/MainWindow.xaml` - Data Folder 설정 카드 추가
- `View/MainWindow.xaml.cs` - ChangeGroupsFolder_Click 핸들러, 경로 표시 초기화
- `BackupHelper.cs` - Groups 추출 경로를 AppPaths.GroupsFolder로 변경, 아이콘 폴백 탐색
- `Strings/en-US/Resources.resw` - 6개 리소스 문자열 추가
- `Strings/ko-KR/Resources.resw` - 6개 리소스 문자열 추가
- `Strings/ja-JP/Resources.resw` - 6개 리소스 문자열 추가
- `Strings/zh-CN/Resources.resw` - 6개 리소스 문자열 추가
- `Strings/zh-TW/Resources.resw` - 6개 리소스 문자열 추가

**검증 결과**
- 빌드: 성공 (경고 767개 / 오류 0개) - 모두 기존 nullable 경고
- 포맷팅: dotnet format 완료

**제한 사항**
- 그룹명 변경 후 작업 표시줄 아이콘: `%AppData%` 가상화로 pinned .lnk 수정 불가, 재고정 필요
- MSIX 패키징 후 실제 VFS 동작 검증 필요

### 코드 리뷰 버그 수정 (Groups 폴더 변경)

**작업 일자**: 2026-02-14

**수행한 작업 요약**
- `ChangeGroupsFolder_Click`에서 동일 경로 선택 시 불필요한 복사/설정 저장 방지
- 기본 경로(`%USERPROFILE%\AppGroup`) 선택 시 `GroupsDataPath`를 빈 문자열로 리셋하여 기본값 복원 가능하게 수정
- `CopyDirectoryRecursive` 중복 정의 제거: `AppPaths.cs`의 메서드를 `internal`로 공개하여 `MainWindow.xaml.cs`에서 재사용

**변경된 파일 목록**
- `AppPaths.cs` - `CopyDirectoryRecursive` 접근 제한자 `private` → `internal`
- `View/MainWindow.xaml.cs` - 동일 경로/기본 경로 검증 추가, 중복 `CopyDirectoryRecursive` 제거, `AppPaths.CopyDirectoryRecursive` 호출

**검증 결과**
- 빌드: 성공 (경고 767개 / 오류 0개) - 기존과 동일

---

## 2026-02-13

### 업데이트 확인 시 Store 앱 페이지 열기로 변경

**작업 일자**: 2026-02-13

**수행한 작업 요약**
- 업데이트 발견 시 DownloadAndInstallStoreUpdatesAsync 대신 ms-windows-store://pdp/ URI로 Store 앱 페이지 열기
- "다운로드 중..." 문구 제거, "업데이트가 있습니다." 문구만 표시
- SettingsHelper.DownloadAndInstallStoreUpdatesAsync() 메서드 제거
- 5개 언어 리소스에서 UpdateInstalled, UpdateCanceled, UpdateFailed 키 제거

**변경된 파일**
- `View/MainWindow.xaml.cs` - CheckForUpdatesButton_Click에서 Store 앱 페이지 열기로 변경
- `SettingsHelper.cs` - DownloadAndInstallStoreUpdatesAsync 메서드 제거
- `Strings/en-US/Resources.resw` - UpdateAvailable 값 변경, 미사용 리소스 3개 제거
- `Strings/ko-KR/Resources.resw` - 동일
- `Strings/ja-JP/Resources.resw` - 동일
- `Strings/zh-CN/Resources.resw` - 동일
- `Strings/zh-TW/Resources.resw` - 동일
...
### 업데이트 확인 기능을 정보(About) 화면으로 이동

**작업 일자**: 2026-02-13

**수행한 작업 요약**
- Settings 탭의 Update Section을 About 탭으로 이동
- About 카드와 Open Source Licenses 사이에 배치
- 스타일을 AboutCardStyle로 변경

**변경된 파일**
- `View/MainWindow.xaml` - Settings 콘텐츠에서 Update Section 제거, About 콘텐츠에 Update Section 추가
...
### 설정 화면에 업데이트 확인 기능 추가

**작업 일자**: 2026-02-13

**수행한 작업 요약**
- About 탭에 Microsoft Store 업데이트 확인 카드 UI 추가
- SettingsHelper에 CheckForStoreUpdatesAsync(), DownloadAndInstallStoreUpdatesAsync() 정적 메서드 추가
- Windows.Services.Store.StoreContext API 사용하여 업데이트 확인/다운로드/설치
- 업데이트 확인 중 ProgressRing 표시, 결과에 따른 상태 메시지 표시
- 패키지 앱이 아닌 경우 안전하게 false 반환

**변경된 파일**
- `SettingsHelper.cs` - using Windows.Services.Store 추가, CheckForStoreUpdatesAsync(), DownloadAndInstallStoreUpdatesAsync() 메서드 추가
- `View/MainWindow.xaml` - 설정 콘텐츠에 Update Section 카드 추가 (Button + ProgressRing + TextBlock)
- `View/MainWindow.xaml.cs` - CheckForUpdatesButton_Click 이벤트 핸들러 추가
- `Strings/en-US/Resources.resw` - CheckForUpdates, CheckForUpdatesDescription, CheckForUpdatesButton, UpdateAvailable, NoUpdatesAvailable, UpdateInstalled, UpdateCanceled, UpdateFailed, UpdateCheckError 추가
- `Strings/ko-KR/Resources.resw` - 한국어 번역 추가
- `Strings/ja-JP/Resources.resw` - 일본어 번역 추가
- `Strings/zh-CN/Resources.resw` - 중국어 간체 번역 추가
- `Strings/zh-TW/Resources.resw` - 중국어 번체 번역 추가

**검증 결과**
- 빌드: 성공 (오류 0개)

**제한 사항**
- Microsoft Store를 통해 배포된 MSIX 패키지 앱에서만 업데이트 확인 가능
- 사이드로드 또는 개발 빌드에서는 업데이트 확인이 작동하지 않음

---

### 숨김 파일 및 폴더 표시 설정 기능 추가

**작업 일자**: 2026-02-13

**수행한 작업 요약**
- StartMenuSettingsDialog에 숨김 파일/폴더 표시 On/Off 토글 추가
- 설정 On 시 FolderContentsPopupWindow에서 숨김 파일 및 폴더가 표시되도록 필터링 로직 추가
- FileAttributes.Hidden 속성 기준으로 필터링
- 기본값: Off (숨김 파일/폴더 미표시)

**변경된 파일**
- `SettingsHelper.cs` - AppSettings 클래스에 `ShowHiddenFilesAndFolders` 속성 추가 (기본값: false)
- `View/StartMenuSettingsDialog.xaml` - FolderDisplaySection에 ShowHiddenFilesAndFoldersToggle 추가
- `View/StartMenuSettingsDialog.xaml.cs` - 토글 로드/저장 로직 및 `ShowHiddenFilesAndFoldersToggle_Toggled` 이벤트 핸들러 추가
- `View/FolderContentsPopupWindow.xaml.cs` - `LoadFolderContents` 메서드에 설정 기반 필터링 로직 추가 (.Where 절로 Hidden 속성 검사)
- `Strings/en-US/Resources.resw` - ShowHiddenFilesAndFolders, ShowHiddenFilesAndFoldersDescription 추가
- `Strings/ko-KR/Resources.resw` - 한국어 번역 추가
- `Strings/ja-JP/Resources.resw` - 일본어 번역 추가
- `Strings/zh-CN/Resources.resw` - 중국어 간체 번역 추가
- `Strings/zh-TW/Resources.resw` - 중국어 번체 번역 추가
...
### MSIX 파일시스템 가상화 비활성화 및 데이터 마이그레이션

MSIX 패키지 앱에서 VFS(Virtual File System) 드라이버가 커널 수준에서 I/O를 가로채 `%LocalAppData%\Packages\{PFN}\LocalCache\Local\`로 리다이렉트하는 문제 해결. `uap10:FileSystemWriteVirtualization` 비활성화 선언과 기존 데이터 마이그레이션 로직 추가.

#### Step 1: Package.appxmanifest VFS 비활성화 선언
- `desktop6` 네임스페이스 추가 (`http://schemas.microsoft.com/appx/manifest/desktop/windows10/6`)
- `<Properties>`에 `desktop6:FileSystemWriteVirtualization`, `desktop6:RegistryWriteVirtualization` 비활성화
- `<Capabilities>`에 `unvirtualizedResources` 추가
- 주의: `uap10:`이나 `virtualization:` 네임스페이스가 아닌 `desktop6:` 사용 (SDK XSD 스키마 검증 기준)
  - `desktop6:` = 단순 enabled/disabled 값 (DesktopManifestSchema_v6.xsd)
  - `virtualization:` = 복합 타입 (ExcludedDirectories 하위 요소, VirtualizationManifestSchema.xsd)

#### Step 2: AppPaths.cs 데이터 마이그레이션 로직
- `MigrateFromPackageFolderIfNeeded()` 정적 메서드 추가 (~80행)
- `Package.Current.Id.FamilyName`으로 패키지 가상화 경로 구성
- 마이그레이션 대상: appgroups.json, settings.json, startmenu.json, lastEdit, lastOpen, Groups/, Icons/
- 충돌 방지: 실제 경로에 appgroups.json이 이미 있으면 스킵
- 완료 표시: `.migrated` 파일 생성으로 일회성 실행 보장
- `GetPackageFamilyName()`: 패키지 환경 판별 헬퍼
- `CopyDirectoryRecursive()`: 재귀 디렉터리 복사 (기존 파일 건너뜀)

#### Step 3: Program.cs 마이그레이션 호출
- `Main()` 최상단에 `AppPaths.MigrateFromPackageFolderIfNeeded()` 호출 추가
- try/catch로 감싸서 마이그레이션 실패 시에도 앱 정상 시작

#### 변경된 파일
- `Package.appxmanifest` - desktop6 네임스페이스, VFS 비활성화 속성, unvirtualizedResources 케이퍼빌리티
- `AppPaths.cs` - MigrateFromPackageFolderIfNeeded(), GetPackageFamilyName(), CopyDirectoryRecursive() 추가 (~80행)
- `Program.cs` - Main() 최상단에 마이그레이션 호출 3행 추가

#### 검증 결과
- 빌드: 성공 (오류 0개)
- README.md: 갱신 (MSIX VFS 비활성화 및 마이그레이션 설명 추가)

---

### AllApps 아이콘 추출 품질 개선

AllApps 다이얼로그에서 일부 Win32 앱 아이콘이 32x32로 작게 표시되는 문제 수정. shell:AppsFolder에서 일부 Win32 앱의 item.Path가 exe 경로 대신 AUMID를 반환하여 저품질 아이콘 추출 경로를 타는 것이 원인.

#### Step 1: ExtractIconFromPidl PIDL 구성 버그 수정
- **IconHelper.UwpExtractor.cs** - `ExtractIconFromPidl` 시그니처에 `IntPtr parentAbsolutePidl` 파라미터 추가
- `ILCombine(IntPtr.Zero, pidl)` → `ILCombine(parentAbsolutePidl, pidl)`로 수정하여 절대 PIDL 생성
- `TryExtractIconViaShellFolder` 호출부에서 `appsFolderPidl` 전달
- `IExtractIcon::Extract` 아이콘 요청 사이즈 256→48로 변경
- `IShellItemImageFactory::GetImage` 폴백 사이즈 256→48로 변경
- `TryExtractIconFromShellPath` 아이콘 요청 사이즈 256→48로 변경

#### Step 2: TryGetExePathFromAumid exe 경로 해석 개선
- **View/EditGroupWindow.AllApps.cs** - 3가지 방법으로 exe 경로 검색
  1. `IShellItem.GetDisplayName(SIGDN.FILESYSPATH)` - Shell API로 파일 시스템 경로 추출 (Win32 앱)
  2. `ActivatableClasses\Package` 레지스트리 (기존 방법, MSIX/UWP 패키지)
  3. `App Paths` 레지스트리 (HKLM) - AUMID 기반 이름 매칭으로 exe 경로 검색

#### Step 3: GetAppIconFromShellAsync 아이콘 품질 확인
- **View/EditGroupWindow.AllApps.cs** - IconCache 반환 아이콘의 크기를 Bitmap으로 확인
- 40x40 미만이면 폴백 경로(shortcut 방식) 계속 시도
- 크기 확인 실패 시 기존 아이콘 그대로 사용

#### 변경된 파일
- `IconHelper.UwpExtractor.cs` - ExtractIconFromPidl 시그니처/PIDL 수정, 아이콘 사이즈 변경
- `View/EditGroupWindow.AllApps.cs` - TryGetExePathFromAumid 확장, GetAppIconFromShellAsync 품질 확인

#### 검증 결과
- 빌드: 성공 (dotnet build, x64)
- README.md 갱신: 불필요 (내부 아이콘 추출 로직 개선, 외부 인터페이스 변경 없음)
